-- 05_IA_FALLBACK_FIX_V2.sql
-- Este script corrige el error de mezcla de temas y garantiza el conteo exacto.

-- 1. RPC: count_available_questions (CORRECCIÓN: Filtrado exacto por tema)
...
BEGIN
    ...
    ELSE
        ...
        WHERE q.estado = 'active'
          AND m.nombre IN (SELECT jsonb_array_elements_text(p_topics));
    END IF;

    -- Conteo por Tema (para la UI - Asegura que no mezcle si se pide uno específico)
...
-- 2. RPC: start_session (CORRECCIÓN: Selección estricta del tema elegido)
...
BEGIN
    ...
    VALUES (
        ...
        CASE WHEN p_examen_id IS NOT NULL THEN 'simulacro'::exam_mode ELSE 'materia'::exam_mode END,
        p_config,
        0
    )
    RETURNING id INTO v_intento_id;

    -- Poblamiento RIGUROSO por tema
    IF p_examen_id IS NOT NULL THEN
